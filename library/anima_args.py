"""Command-line arguments and configuration helpers for Anima training."""
...
def add_anima_model_arguments(parser: argparse.ArgumentParser):
    parser.add_argument(
        "--pretrained_model_name_or_path",
        type=str,
        default=None,
        help="Anima DiT checkpoint to train / 学習対象の Anima DiT チェックポイント",
    )
...
def add_optimizer_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--fused_adamw",
        action="store_true",
        help="Use torch.optim.AdamW's fused implementation. Intended for LoRA training when the extra fp32 optimizer state fits in VRAM.",
    )
    ...
    parser.add_argument(
        "--fused_backward_pass",
        action="store_true",
        help="Combines backward pass and the optimizer step to reduce VRAM usage with Anima. Currently requires Adafactor.",
    )
    parser.add_argument(
...
def add_training_arguments(parser: argparse.ArgumentParser, support_dreambooth: bool):
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="minimum flow timestep for Anima training (0-999)",
    )
    parser.add_argument(
        "--max_timestep",
        type=int,
        default=None,
        help="maximum flow timestep for Anima training (1-1000)",
    )
    parser.add_argument(
        "--loss_type",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Huber threshold schedule used by Anima.",
    )
    parser.add_argument(
        "--huber_c",
        type=float,
...
def add_dataset_arguments(
    ...
    parser.add_argument(
        "--bucket_free_fit",
        action="store_true",
        help="create a near-native-aspect bucket for each image at the configured target pixel area; allows upscaling and minimizes crop. Cannot be combined with bucket_no_upscale"
        + " / 按配置的目标像素面积为每张图生成近似原生宽高比的bucket，允许放大并尽量减少裁剪。不能与bucket_no_upscale同时使用",
    )
    parser.add_argument(
...
def add_anima_saving_arguments(parser: argparse.ArgumentParser):
    parser.add_argument(
        "--save_model_as",
        type=str,
        default="safetensors",
        choices=["safetensors"],
        help="Anima checkpoints are saved as safetensors.",
    )
